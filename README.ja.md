...
- ドラッグで任意の範囲を選択。サイズ表示と拡大鏡でピクセル単位の調整が可能
...
### 範囲録画
- 専用のグローバルホットキー（既定 **F2**）：ドラッグで録画範囲を選択して開始、もう一度押す（または停止ボタン）で終了
...
4. **コピー / 保存 / ピン留め**で出力。`Esc` でキャプチャを終了（注釈がある場合は確認ダイアログあり、「今後は確認しない」も選択可）。
